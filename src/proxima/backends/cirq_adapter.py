...
class CirqBackendAdapter(BaseBackendAdapter):
	...
	def validate_circuit(self, circuit: Any) -> ValidationResult:
		...
		if not isinstance(circuit, getattr(cirq, "Circuit", tuple())):
	...
	def estimate_resources(self, circuit: Any) -> ResourceEstimate:
		if not self.is_available():
			return ResourceEstimate(memory_mb=None, time_ms=None, metadata={"reason": "cirq not installed"})
		try:
			import cirq
		except Exception:
			return ResourceEstimate(memory_mb=None, time_ms=None, metadata={"reason": "cirq import failed"})

		if not isinstance(circuit, getattr(cirq, "Circuit", tuple())):
			return ResourceEstimate(memory_mb=None, time_ms=None, metadata={"reason": "not a cirq.Circuit"})
