...
class QiskitBackendAdapter(BaseBackendAdapter):
	...
	def is_available(self) -> bool:
		return importlib.util.find_spec("qiskit") is not None and importlib.util.find_spec("qiskit_aer") is not None
	...
	def estimate_resources(self, circuit: Any) -> ResourceEstimate:
		if not self.is_available():
			return ResourceEstimate(memory_mb=None, time_ms=None, metadata={"reason": "qiskit/qiskit-aer not installed"})
		try:
			from qiskit import QuantumCircuit
		except Exception:
			return ResourceEstimate(memory_mb=None, time_ms=None, metadata={"reason": "qiskit import failed"})

		if not isinstance(circuit, QuantumCircuit):
			return ResourceEstimate(memory_mb=None, time_ms=None, metadata={"reason": "not a QuantumCircuit"})
